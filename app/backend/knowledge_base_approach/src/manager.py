...
if __name__=="__main__":
    ...
    parser.add_argument("--data-dir", type=str, default=None,
        help="Specify directory of data"
    )
    ...
    args.proc_dir  = f"{output_dir}/data/"+args.task_meta+"/"
    args.outp_dir  = f"{output_dir}/output/"+args.task_meta+"/"
    args.clss_dir  = f"{output_dir}/classified/"+args.task_meta+"/"
    args.beam_mode = 'IE' if config['task_abbr']=='oie' else 'RC'
    args.ner_mode  = 'np' if config['task_abbr']=='oie' else 'rc'
    args.part      = '0'
    
    if args.clean_history and args.stage==0 and os.path.exists(f"{output_dir}/"):
        shutil.rmtree(f"{output_dir}/"); shutil.rmtree(f"{runs_dir}/")
    Create(f"{runs_dir}/")
    Create(f"{result_dir}/")
    Create(f"{output_dir}/")
    Create(f"{output_dir}/data/")
    Create(f"{output_dir}/output/")
    Create(f"{output_dir}/classified/")
    ...
    if args.stage<=2 and (args.stage==2 or not args.debug):
        RESULT = f"{result_dir}/" + ".".join([args.task,args.model,args.ner_mode,f"d{args.max_distance}",f"b{args.beam_size}"])
        SysCall(
    ...
    if args.stage<=3 and (args.stage==3 or not args.debug) and (args.task_meta in ['FewRel', 'TACRED']):
        RESULT = f"{result_dir}/" + ".".join([args.task,args.model,args.ner_mode,f"d{args.max_distance}",f"b{args.beam_size}"]); Create("scripts/rc/data/")
        SysCall(
